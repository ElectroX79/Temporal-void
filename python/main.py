...
def record_sensor_samples(celsius: float, humidity: float, lux: float):
    """Callback invoked by the board sketch via Bridge.notify.
    Stores and forwards temperature, humidity, and light samples.
    """
    if celsius is None or humidity is None or lux is None:
        print("Received invalid sensor samples: celsius=%s, humidity=%s, lux=%s" % (celsius, humidity, lux))
        return
    ...
    db.write_sample("light", float(lux), ts)
    ...
    ui.send_message('light', {"value": float(lux), "ts": ts})
    ...
    # Store and forward derived metrics
